...
a = angular.module("dualmultiselect", []), a.directive("dualmultiselect", [function() {
  return {
    ...
    template:    ' <div class="dualmultiselect"> '
              ...
              +  '          <div class="pool"> '
              +  '              <ul> '
              +  '                <a><li ng-repeat="item in options.items | filter: searchTerm | orderBy: options.orderProperty" ng-click="transfer(options.items, options.selectedItems, options.items.indexOf(item))"> '
              +  '                  {{item.name}}&nbsp;&rArr; '
              +  '                </li></a>'
              +  '              </ul> '
              +  '          </div> '
              ...
              +  '         <div class="pool"> '
              +  '           <ul> '
              +  '             <a><li ng-repeat="item in options.selectedItems | orderBy: options.orderProperty" ng-click="transfer(options.selectedItems, options.items, options.selectedItems.indexOf(item))"> '
              +  '                &lArr;&nbsp;{{item.name}} '
              +  '             </li></a> '
              +  '           </ul> '
              +  '         </div> '
